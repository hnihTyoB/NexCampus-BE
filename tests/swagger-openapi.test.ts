...
describe("Auto Swagger OpenAPI Generation via zod-to-openapi", () => {
  ...
  it("should register standard security schemes (BearerAuth and ApiKeyAuth)", () => {
    ...
    assert.equal(
      swaggerSpec.components.securitySchemes.BearerAuth.type,
      "http",
    );
    assert.equal(
    ...
    assert.equal(
      swaggerSpec.components.securitySchemes.ApiKeyAuth.type,
      "apiKey",
    );
    assert.equal(
      swaggerSpec.components.securitySchemes.ApiKeyAuth.name,
      "x-api-key",
    );
  });
