...
import { requestDeactivateSchema, confirmDeactivateSchema } from '../src/modules/auth/auth.validation';
...
describe('Auth Deactivation Test Suite (GDPR Self-Deactivation)', () => {
  ...
  let dispatchedEmails: Array<{ email: string; token: string; fullName?: string }> = [];
  ...
  beforeEach(() => {
    ...
    mockRepository = {
      findById: async (id: string) => {
        if (id === 'user-active-1') {
          return {
            ...
            deletedBy: null,
            roleId: 'role-user-id',
            role: { id: 'role-user-id', name: ROLES.USER, description: null, isSystem: true, createdAt: new Date(), updatedAt: new Date() },
            createdAt: new Date(),
            updatedAt: new Date(),
          } as any;
        }
        if (id === 'admin-sole-1') {
          return {
            ...
            role: { id: 'role-admin-id', name: ROLES.ADMIN, description: null, isSystem: true, createdAt: new Date(), updatedAt: new Date() },
            createdAt: new Date(),
            updatedAt: new Date(),
          } as any;
        }
      ...
      createDeactivationToken: async (userId: string, token: string, expiresAt: Date) => ({
        id: 'token-uuid-1',
        token,
        userId,
        expiresAt,
    ...
    mockMailService = {
      sendAccountDeactivationEmail: async (email: string, token: string, fullName?: string) => {
  ...
  describe('1. Zod Validation Schemas', () => {
    it('requestDeactivateSchema requires password and accepts optional reason', () => {
      const valid = { password: 'Password@123', reason: 'Tôi muốn đổi sang dịch vụ khác' };
      assert.equal(requestDeactivateSchema.safeParse(valid).success, true);

      const validWithoutReason = { password: 'Password@123' };
      assert.equal(requestDeactivateSchema.safeParse(validWithoutReason).success, true);

      const missingPassword = { reason: 'No pass' };
      assert.equal(requestDeactivateSchema.safeParse(missingPassword).success, false);

      const tooLongReason = { password: 'pass', reason: 'a'.repeat(501) };
      assert.equal(requestDeactivateSchema.safeParse(tooLongReason).success, false);
    });

    it('confirmDeactivateSchema requires token', () => {
      assert.equal(confirmDeactivateSchema.safeParse({ token: 'abc-123' }).success, true);
      assert.equal(confirmDeactivateSchema.safeParse({ token: '' }).success, false);
  ...
  describe('2. Request Deactivation Flow (POST /auth/deactivate/request)', () => {
    it('should reject if wrong password is provided', async () => {
      await assert.rejects(
        authService.requestDeactivate('user-active-1', { password: 'WrongPassword!' }),
        (err: any) => {
    ...
    it('should reject if user account is already inactive or soft-deleted', async () => {
      await assert.rejects(
        authService.requestDeactivate('user-inactive-1', { password: 'Password@123' }),
        (err: any) => {
          assert.equal(err instanceof AppError, true);
    ...
    it('should reject if user is a social user without password', async () => {
      await assert.rejects(
        authService.requestDeactivate('user-social-1', { password: 'AnyPassword' }),
        (err: any) => {
          assert.equal(err instanceof AppError, true);
    ...
    it('should prevent deactivation if user is the sole active ADMIN (Anti-lockout)', async () => {
      await assert.rejects(
        authService.requestDeactivate('admin-sole-1', { password: 'Admin@123' }),
        (err: any) => {
          assert.equal(err instanceof AppError, true);
    ...
    it('should successfully generate token, dispatch email and log audit when password matches', async () => {
      let createdToken = '';
      mockRepository.createDeactivationToken = async (userId: string, token: string, expiresAt: Date) => {
        createdToken = token;
        const ttlMs = expiresAt.getTime() - Date.now();
        assert.ok(ttlMs > 14 * 60 * 1000 && ttlMs <= 15 * 60 * 1000, 'Expiry should be 15 minutes');
        return {} as any;
      };
      ...
      assert.equal(auditLogs[0].action, AUDIT_ACTION.REQUEST_ACCOUNT_DEACTIVATION);
  ...
  describe('3. Confirm Deactivation Flow (POST /auth/deactivate/confirm)', () => {
    ...
    it('should successfully deactivate user, revoke all sessions, invalidate cache and record audit log', async () => {
      ...
      mockRepository.deactivateUserAndRevokeSessions = async (userId: string, tokenId: string) => {
      ...
      assert.equal((permissionCacheService as any).userCache.has('user-active-1'), false);

      // Audit log should be created
      assert.equal(auditLogs.length, 1);
      assert.equal(auditLogs[0].action, AUDIT_ACTION.CONFIRM_ACCOUNT_DEACTIVATION);
      assert.equal(auditLogs[0].targetId, 'user-active-1');
  ...
  describe('4. OpenAPI Documentation Registration', () => {
    it('should register /auth/deactivate/request and /auth/deactivate/confirm in OpenAPI specification', () => {
      assert.ok(swaggerSpec.paths['/auth/deactivate/request'], 'Endpoint /auth/deactivate/request must be registered');
      assert.ok(swaggerSpec.paths['/auth/deactivate/request'].post, 'POST /auth/deactivate/request must exist');
      assert.equal(swaggerSpec.paths['/auth/deactivate/request'].post.tags[0], 'Auth');

      assert.ok(swaggerSpec.paths['/auth/deactivate/confirm'], 'Endpoint /auth/deactivate/confirm must be registered');
      assert.ok(swaggerSpec.paths['/auth/deactivate/confirm'].post, 'POST /auth/deactivate/confirm must exist');
      assert.equal(swaggerSpec.paths['/auth/deactivate/confirm'].post.tags[0], 'Auth');
    });
  });
});
