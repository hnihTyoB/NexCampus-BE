...
describe("URL Helper — isPublicHttpUrl", () => {
  ...
  it("should return true for localhost when allowPrivate: true (dev mode)", () => {
    assert.equal(
      isPublicHttpUrl("http://localhost/app", { allowPrivate: true }),
      true,
    );
  });
  ...
  it("should return true for private IP when allowPrivate: true (dev mode)", () => {
    assert.equal(
      isPublicHttpUrl("http://192.168.1.100/api", { allowPrivate: true }),
      true,
    );
  });
  ...
  it("should return false for http://localhost in production mode", () => {
    assert.equal(
      isPublicHttpUrl("http://localhost/evil", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for http://127.0.0.1 in production mode", () => {
    assert.equal(
      isPublicHttpUrl("http://127.0.0.1:8080/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for http://127.255.255.255 (loopback range)", () => {
    assert.equal(
      isPublicHttpUrl("http://127.255.255.255/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for http://0.0.0.0", () => {
    assert.equal(
      isPublicHttpUrl("http://0.0.0.0/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for 10.x private range", () => {
    assert.equal(
      isPublicHttpUrl("http://10.0.0.1/secret", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for 172.16.x (private class B start)", () => {
    assert.equal(
      isPublicHttpUrl("http://172.16.0.5/api", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for 172.31.x (private class B end)", () => {
    assert.equal(
      isPublicHttpUrl("http://172.31.255.255/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return true for 172.15.x (not private — outside 172.16–172.31 range)", () => {
    ...
    assert.equal(
      isPublicHttpUrl("http://172.15.0.1/", { allowPrivate: false }),
      true,
    );
  });
  ...
  it("should return false for 192.168.x.x private class C", () => {
    assert.equal(
      isPublicHttpUrl("http://192.168.1.1/admin", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for 169.254.x.x link-local (AWS instance metadata)", () => {
    assert.equal(
      isPublicHttpUrl("http://169.254.169.254/latest/meta-data/", {
        allowPrivate: false,
      }),
      false,
    );
  ...
  it("should return false for IPv6 loopback ::1", () => {
    assert.equal(
      isPublicHttpUrl("http://[::1]/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for IPv6 ULA fc00::1", () => {
    assert.equal(
      isPublicHttpUrl("http://[fc00::1]/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for IPv6 ULA fd00::1", () => {
    assert.equal(
      isPublicHttpUrl("http://[fd00::1]/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for IPv6 link-local fe80::1", () => {
    assert.equal(
      isPublicHttpUrl("http://[fe80::1]/", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for ftp:// scheme regardless of allowPrivate", () => {
    assert.equal(
      isPublicHttpUrl("ftp://example.com/file.txt", { allowPrivate: true }),
      false,
    );
  });
  ...
  it("should return false for file:// scheme regardless of allowPrivate", () => {
    assert.equal(
      isPublicHttpUrl("file:///etc/passwd", { allowPrivate: true }),
      false,
    );
  });
  ...
  it("should return false for javascript: scheme regardless of allowPrivate", () => {
    assert.equal(
      isPublicHttpUrl("javascript:alert(1)", { allowPrivate: true }),
      false,
    );
  });
  ...
  it("should return false for data: URI regardless of allowPrivate", () => {
    assert.equal(
      isPublicHttpUrl("data:text/html,<h1>XSS</h1>", { allowPrivate: true }),
      false,
    );
  });
  ...
  it("should return false for bare hostname without dot (intranet)", () => {
    assert.equal(
      isPublicHttpUrl("http://intranet/resource", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for .local TLD", () => {
    assert.equal(
      isPublicHttpUrl("http://app.local/path", { allowPrivate: false }),
      false,
    );
  });
  ...
  it("should return false for .internal TLD", () => {
    assert.equal(
      isPublicHttpUrl("http://service.internal/api", { allowPrivate: false }),
      false,
    );
  });
...
describe("URL Helper — extractDomain", () => {
  it("should extract hostname from valid URL", () => {
    assert.equal(
      extractDomain("https://api.example.com/v1/users"),
      "api.example.com",
    );
  });
...
describe("SSRF Protection — resolveAndValidateDns (DNS Rebinding Defense)", () => {
  ...
  it("should block when public domain resolves to AWS metadata IP (169.254.169.254)", async () => {
    const mockLookup = async () => [{ address: "169.254.169.254", family: 4 }];
  ...
  it("should return error when DNS lookup fails (NXDOMAIN / ENOTFOUND)", async () => {
    ...
    const result = await resolveAndValidateDns(
      "invalid-non-existent-domain.xyz",
      {
        dnsLookup: mockLookup,
      },
    );
...
describe("SSRF Protection — resolveSafeRedirectChain (HTTP Redirect Chaining Defense)", () => {
  it("should pass for direct URL with no redirects (HTTP 200)", async () => {
    ...
    const result = await resolveSafeRedirectChain(
  ...
  it("should safely follow valid public redirect chain (HTTP 301 -> 302 -> 200)", async () => {
    ...
    const mockFetcher = async (url: string) => {
      if (url === "https://step1.com/start") {
        return {
          status: 301,
          headers: { location: "https://step2.com/middle" },
        };
      }
      if (url === "https://step2.com/middle") {
        return {
          status: 302,
          headers: { location: "https://final.com/resource" },
        };
      }
  ...
  it("should block when public domain redirects to private IP (127.0.0.1 / localhost)", async () => {
    ...
    const mockFetcher = async (url: string) => {
      if (url === "https://evil-redirector.com/link") {
        return {
          status: 302,
          headers: { location: "http://127.0.0.1:8888/admin" },
        };
      }
      return { status: 200, headers: {} };
    };
    ...
    const result = await resolveSafeRedirectChain(
      "https://evil-redirector.com/link",
      {
        allowPrivate: false,
        dnsLookup: mockLookup,
        httpFetcher: mockFetcher,
    ...
    assert.equal(result.isValid, false);
    assert.ok(
      result.reason?.includes("127.0.0.1") ||
        result.reason?.includes("private"),
    );
  });
  ...
  it("should block when public domain redirects to AWS Metadata IP (169.254.169.254)", async () => {
    ...
    assert.equal(result.isValid, false);
    assert.ok(
      result.reason?.includes("169.254.169.254") ||
        result.reason?.includes("private"),
    );
  });
  ...
  it("should block when redirect target uses disallowed protocol (file:, ftp:, javascript:)", async () => {
    ...
    assert.equal(result.isValid, false);
    assert.ok(
      result.reason?.includes("Disallowed protocol") ||
        result.reason?.includes("file:"),
    );
  });
  ...
  it("should detect and prevent circular redirect loops (A -> B -> A)", async () => {
    ...
    const mockFetcher = async (url: string) => {
      if (url === "https://loop-a.com/start") {
        return {
          status: 302,
          headers: { location: "https://loop-b.com/next" },
        };
      }
      if (url === "https://loop-b.com/next") {
        return {
          status: 302,
          headers: { location: "https://loop-a.com/start" },
        };
      }
      return { status: 200, headers: {} };
    };
  ...
  it("should block when redirect count exceeds maxRedirects limit", async () => {
    ...
    const mockFetcher = async () => {
      ...
      return {
        status: 302,
        headers: { location: `https://infinite-redirect.com/step-${counter}` },
      };
    };

    const result = await resolveSafeRedirectChain(
      "https://infinite-redirect.com/start",
      {
        maxRedirects: 3,
        allowPrivate: false,
        dnsLookup: mockLookup,
