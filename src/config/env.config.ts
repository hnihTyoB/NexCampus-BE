...
const envSchema = z
  .object({
    ...
    R2_PRESIGNED_URL_EXPIRES_IN_SECONDS: z.coerce.number().int().positive().default(300),
    R2_AVATAR_MAX_FILE_SIZE_MB: z.coerce.number().positive().default(5),

    // ── Notification Worker ───────────────────────────────────────────────────
    NOTIFICATION_WORKER_ENABLED: z.string().default('true'),
    NOTIFICATION_WORKER_INTERVAL_MS: z.coerce.number().int().positive().default(60000),
  ...
  .refine(
    ...
    { message: 'JWT_ACCESS_SECRET must be securely set in production (không được dùng giá trị mặc định)', path: ['JWT_ACCESS_SECRET'] },
  )
  .refine(
    (env) => {
      if (env.NODE_ENV !== 'production') return true;
    ...
    { message: 'JWT_REFRESH_SECRET must be securely set in production (không được dùng giá trị mặc định)', path: ['JWT_REFRESH_SECRET'] },
  )
  .refine(
    (env) => {
      if (env.NODE_ENV !== 'production') return true;
    ...
    { message: 'ALLOWED_ORIGINS must be explicitly set with trusted domains in production (wildcard * is not allowed)', path: ['ALLOWED_ORIGINS'] },
  )
  .refine(
    (env) => {
      if (env.NODE_ENV !== 'production') return true;
    ...
    { message: 'ENCRYPTION_KEY or APP_SECRET must be securely set in production', path: ['ENCRYPTION_KEY'] },
  );
