...
import { LoginDto, LoginResponseDto, AuthTokensDto, MeDto, RegisterDto, UpdateProfileDto, UpdatePasswordDto, ForgotPasswordDto, ResetPasswordDto, ResendVerificationDto, GetAvatarUploadUrlDto, GetAvatarUploadUrlResponseDto, ConfirmAvatarUploadDto, RequestDeactivateDto, ConfirmDeactivateDto, Setup2FAResponseDto, Enable2FADto, Enable2FAResponseDto, Verify2FALoginDto, Disable2FADto, RegenerateBackupCodesDto } from './auth.dto';
...
import { encryptSecret, decryptSecret, hashToken } from '../../common/helpers/crypto.helper';
import { generateTotpSecret, generateOtpauthUri, verifyTotpCode, generateBackupCodes } from '../../common/helpers/totp.helper';
...
export class AuthService {
  ...
  async setup2FA(userId: string): Promise<Setup2FAResponseDto> {
    ...
    if (user.twoFactorEnabled) {
      throw new AppError('Xác thực 2 bước đã được kích hoạt trên tài khoản này', 400, ERROR_CODE.TWO_FACTOR_ALREADY_ENABLED);
    }
  ...
  async enable2FA(
    userId: string,
    data: Enable2FADto,
    metadata?: { ipAddress?: string; userAgent?: string; currentRefreshToken?: string },
    ...
    if (user.twoFactorEnabled) {
      throw new AppError('Xác thực 2 bước đã được kích hoạt trước đó', 400, ERROR_CODE.TWO_FACTOR_ALREADY_ENABLED);
    }

    const isValid = verifyTotpCode(data.secret, data.code);
    if (!isValid) {
      throw new AppError('Mã xác thực TOTP không chính xác hoặc đã hết hạn', 400, ERROR_CODE.TWO_FACTOR_INVALID_CODE);
    }
    ...
    await this.repository.revokeOtherSessions(userId, metadata?.currentRefreshToken);
  ...
  async verify2FALogin(
    ...
    try {
      ...
      throw new AppError('Mã phiên xác thực 2FA không hợp lệ hoặc đã hết hạn', 401, ERROR_CODE.TOKEN_EXPIRED);
    }

    if (payload.purpose !== '2FA_VERIFICATION' || !payload.id) {
      throw new AppError('Token xác thực 2FA không đúng mục đích', 401, ERROR_CODE.TOKEN_INVALID);
    }

    const user = await this.repository.findById(payload.id);
    if (!user || !user.isActive || !user.twoFactorEnabled || !user.twoFactorSecret) {
      throw new AppError('Tài khoản không khả dụng hoặc chưa kích hoạt 2FA', 400, ERROR_CODE.INVALID_CREDENTIALS);
    }
    ...
    if (!isCodeValid && user.twoFactorBackupCodes && Array.isArray(user.twoFactorBackupCodes)) {
      ...
      const matchIndex = backupCodes.findIndex((storedHash) => {
        ...
        return bufInput.length === bufStored.length && crypto.timingSafeEqual(bufInput, bufStored);
      });
    ...
    if (!isCodeValid) {
      throw new AppError('Mã xác thực 2FA hoặc mã dự phòng không chính xác', 400, ERROR_CODE.TWO_FACTOR_INVALID_CODE);
    }

    // Cấp Access Token + Refresh Token chính thức
    const tokenPayload = { id: user.id, email: user.email, role: user.role.name, roleId: user.roleId };
  ...
  async disable2FA(
    ...
    if (!user.twoFactorEnabled || !user.twoFactorSecret) {
      throw new AppError('Xác thực 2 bước chưa được kích hoạt', 400, ERROR_CODE.VALIDATION_ERROR);
    }

    if (!user.password) {
      throw new AppError('Tài khoản liên kết mạng xã hội không thể tắt 2FA bằng mật khẩu', 400, ERROR_CODE.VALIDATION_ERROR);
    }
  ...
  async regenerateBackupCodes(
    ...
    if (!user.password) {
      throw new AppError('Tài khoản liên kết mạng xã hội không thể tái tạo mã bằng mật khẩu', 400, ERROR_CODE.VALIDATION_ERROR);
    }
