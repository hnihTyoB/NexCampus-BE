...
export const enableMaintenanceSchema = z.object({
  ...
  estimatedEndAt: z
    .string()
    .datetime({
      message: "estimatedEndAt must be a valid ISO 8601 datetime string",
    })
    .nullable()
    .optional(),
  bypassPermissions: z
  ...
  (data) => {
    if (data.startAt && data.estimatedEndAt) {
      return (
        new Date(data.startAt).getTime() < new Date(data.estimatedEndAt).getTime()
      );
    }
    return true;
  },
  {
    message: "estimatedEndAt must be after startAt",
    path: ["estimatedEndAt"],
  },
);
    ...
    (data) => {
      if (data.startAt && data.estimatedEndAt) {
        return (
          ...
          new Date(data.estimatedEndAt).getTime()
        );
      }
      return true;
    },
