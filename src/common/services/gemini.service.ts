...
export class GeminiService implements IAiProvider {
  ...
  private constructor() {
    ...
    if (this.apiKeys.length > 0) {
      console.log(`[GeminiService] Initialized with ${this.apiKeys.length} API keys for rotation/backup.`);
    } else {
      console.warn('[GeminiService] No GEMINI_API_KEY configured — AI features will be unavailable.');
    }
  }
  ...
  async generateJSON<T = Record<string, unknown>>(prompt: string): Promise<T> {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      try {
        ...
        try {
          ...
          console.error('[GeminiService] Raw text response before parse error:', text);
          throw parseError;
        }
      } catch (error) {
        lastError = error;
        const message = error instanceof Error ? error.message : String(error);
        console.warn(`[GeminiService] Key index ${activeIndex} failed:`, message);
        ...
        if (
          ...
          console.log(`🔄 [GeminiService] Chuyển API Key sang index ${this.currentKeyIndex} (Backup Key)...`);
        } else {
    ...
    console.error('[GeminiService] All API keys in the pool failed. Fallback to mock generator. Error:', finalMessage);
    ...
    const mockResponse = {
      ...
      learning: 8.0,
      coding: 7.5,
      comment: 'Thực tập sinh đã thể hiện tinh thần làm việc rất tốt trong tuần qua. Chủ động viết báo cáo hàng ngày đầy đủ chi tiết, nêu rõ tiến độ và khó khăn gặp phải. Bài nộp code có tiến bộ rõ rệt qua các lần chỉnh sửa theo góp ý của Leader.',
      strengths: [
