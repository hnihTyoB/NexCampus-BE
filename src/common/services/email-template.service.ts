...
export class EmailTemplateService {
  ...
  async renderAsync(
    ...
    try {
      const dbTemplate =
        await notificationRepository.findTemplateByCode(templateKey);

      if (dbTemplate && dbTemplate.isActive) {
        const subject = renderTemplateString(
          dbTemplate.subject || "Thông báo từ hệ thống",
          data,
        );
      ...
      console.warn(
        `[EmailTemplateService] Database template fetch failed for '${templateKey}', falling back to built-in layout:`,
        error,
      );
    }
      ...
      default:
        return {
          ...
          html: this.baseLayout(
            (data["title"] as string) || "Thông báo",
            renderTemplateString((data["content"] as string) || "", data),
          ),
        };
    }
  }
  ...
  private resetPassword(data: Record<string, unknown>): EmailTemplate {
    const { resetUrl, fullName } = data as {
      resetUrl: string;
      fullName?: string;
    };
    const safeName = escapeHtml(fullName) || "bạn";
  ...
  private activitySummaryDigest(data: Record<string, unknown>): EmailTemplate {
    ...
    const html = this.baseLayout(
      `📊 Báo cáo Tổng kết Hoạt động (${period})`,
      `
