...
export class EmailWorker {
    ...
    this.activeBatchPromise = (async () => {
      try {
        ...
        if (
          ...
          maintenanceConfig.status === MAINTENANCE_STATUS.MAINTENANCE
        ) {
        ...
        const pending =
          await notificationRepository.claimPendingEmails(BATCH_SIZE);
        ...
        console.log(
          `[EmailWorker] Processing ${pending.length} atomically claimed pending email(s)`,
        );

        await Promise.allSettled(pending.map((record) => this.sendOne(record)));
      } catch (error) {
        console.error(
          "[EmailWorker] Unexpected error during processing:",
          error,
        );
      } finally {
  ...
  private async sendOne(record: ClaimedEmailRecord): Promise<void> {
    try {
      ...
      console.log(
        `[EmailWorker] ✅ Sent email ${record.id} to ${record.toEmail}`,
      );
    } catch (error: any) {
      const newAttempts = record.attempts + 1;
      const newStatus =
        newAttempts >= EMAIL_MAX_ATTEMPTS
          ? EMAIL_STATUS.FAILED
          : EMAIL_STATUS.PENDING;

      await notificationRepository
        .updateEmailStatus(record.id, {
          status: newStatus,
