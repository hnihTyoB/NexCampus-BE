    ...
    try {
      ...
      this.worker = new Worker<CronJobData>(
        ...
        async (job: Job<CronJobData>) => {
          console.log(
            `[CronWorker] ⏳ Processing scheduled job: '${job.data.jobName}'...`,
          );
          await cronService.triggerJob(
            job.data.jobName,
            job.data.params || {},
            {
              actorId: undefined,
      ...
      this.worker.on("completed", (job) => {
        console.log(
          `[CronWorker] ✅ Job '${job.data.jobName}' completed successfully`,
        );
      });

      this.worker.on("failed", (job, err) => {
        console.error(
          `[CronWorker] ❌ Job '${job?.data?.jobName}' failed:`,
          err.message,
        );
      });
