...
async function run() {
  ...
  const loginData = (await loginResp.json()) as { success: boolean; data: { accessToken: string } };
  ...
  console.log(`🤖 Step 2: Requesting AI Suggestion for intern ${internId}, week ${week}...`);
  console.log('(This might take 5-15 seconds as it calls the Gemini API...)');

  const suggestionResp = await fetch(`${BASE_URL}/weekly-evaluations/ai-suggestion`, {
    method: 'POST',
    headers: {
  ...
  console.log(`Leader modified communication score from ${aiData.communication} to ${leaderCommunication}.`);
  ...
  const savePayload = {
    ...
    comment: 'Đã chỉnh sửa điểm giao tiếp của bạn. Hãy cố gắng báo cáo đúng hạn hơn.',
